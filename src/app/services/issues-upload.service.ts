...
export class IssuesUploadService {
  ...
  upload(issues: File): Observable<HttpEvent<any>> {
    const formData: FormData = new FormData();

    formData.append('issues', issues);

    const req = new HttpRequest("POST", AUTH_API+"/uploadIssues", formData, {
  ...
  getFiles(): Observable<any> {
    return this.http.get(AUTH_API+"/issuess/");
  }
}
