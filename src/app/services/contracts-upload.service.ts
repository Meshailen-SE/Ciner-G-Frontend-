...
export class ContractsUploadService {
  ...
  upload(contracts: File): Observable<HttpEvent<any>> {
    const formData: FormData = new FormData();

    formData.append('contracts', contracts);

    const req = new HttpRequest("POST", AUTH_API+"/uploadContracts", formData, {
  ...
  getFiles(): Observable<any> {
    return this.http.get(AUTH_API+"/contractss/");
  }
}
